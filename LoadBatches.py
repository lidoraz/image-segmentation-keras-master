...
def splitImg(im, window=[300, 300]):
    M = window[0]
    N = window[1]
...
def combineImg(imageArr, height, width, window=[300, 300]):
    M = window[0]
    N = window[1]
    outputImg = np.zeros([height, width, 3])
    tileIdx = 0
    for x in range(0, height, M):
        for y in range(0, width, N):
            tile = imageArr[tileIdx]
            tile_X = tile.shape[0]
            tile_Y = tile.shape[1]
            real_M = tile_X if tile_X < M else M
            real_N = tile_Y if tile_Y < N else N
            print(outputImg[x:x + real_M, y:y + real_N].shape[0],outputImg[x:x + real_M, y:y + real_N].shape[1],tile.shape[0],tile.shape[1])
            outputImg[x:x + real_M, y:y + real_N] = tile[0:real_M,0:real_N]
            # print(tileIdx,'/',len(imageArr))
            tileIdx += 1
    return outputImg
